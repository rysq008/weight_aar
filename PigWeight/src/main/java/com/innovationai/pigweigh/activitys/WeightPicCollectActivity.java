...
import com.innovationai.pigweigh.camera.CameraSurfaceView;
import com.innovationai.pigweigh.camera.CameraUtils;
import com.innovationai.pigweigh.camera.ImageUtils;
import com.innovationai.pigweigh.camera.SpiritView;
...
import com.innovationai.pigweigh.utils.UIUtils;
...
//                            LocalBroadcastManager.getInstance(WeightPicCollectActivity.this).sendBroadcast(intent);
                            ...
                            retryPreview();
                            Toast.makeText(WeightPicCollectActivity.this, getErrorMsg(bean.getData().getErrorcode()), Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(WeightPicCollectActivity.this, bean.getMsg(), Toast.LENGTH_SHORT).show();
                    }
    ...
    private static String getErrorMsg(int errorcode) {
        String tipContent = "";
        if (errorcode == ERROR_CODE_PIG_SMALL) {
            tipContent = "离猪远了，手机放低点，请重新拍！";
        } else if (errorcode == ERROR_CODE_PIG_LARGE) {
            tipContent = "离猪近了，手机放高点，请重新拍！";
        } else if (errorcode == ERROR_CODE_NO_FIND_RULER) {
            tipContent = "没尺子或光线太暗，请重新拍！";
        } else if (errorcode == ERROR_CODE_RULER_SMALL) {
            tipContent = "离猪远了，手机放低点，请重新拍！";
        } else if (errorcode == ERROR_CODE_RULER_LARGE) {
            tipContent = "离猪近了，手机放高点，请重新拍！";
        }
        return tipContent;
    }
